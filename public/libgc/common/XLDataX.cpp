...
// 添加64位整型数据
bool XLDataX::PutInt64(unsigned short nKeyID, int64_t nData)
{
    if (m_mapIndexes.find(nKeyID) != m_mapIndexes.end())
    {
        // TODO: add log
...
// 添加Int64数组的内容
bool XLDataX::PutInt64Array(unsigned short nKeyID, int64_t* pnData, int nElements)
{
    if (m_mapIndexes.find(nKeyID) != m_mapIndexes.end())
    {
        // TODO: add log
...
// 获取64位整型数据
bool XLDataX::GetInt64(unsigned short nKeyID, int64_t& nData)
{
    nData = 0;
    map<unsigned short, int>::iterator it = m_mapIndexes.find(nKeyID);
    if (it == m_mapIndexes.end())
...
bool XLDataX::GetInt64ArraySize(unsigned short nKeyID, int& nSize)
{
    ...
    {
        vector<int64_t>& vecTemp = m_vecInt64ArrayItems[nPos];
        nSize = (int)vecTemp.size();
    }
    else
    {
...
bool XLDataX::GetInt64ArrayElement(unsigned short nKeyID, int nIndex, int64_t& nData)
{
    nData = 0;
...
void XLDataX::EncodeInt64ArrayItem(FixedBuffer& fixed_buffer, int nPos)
{
    ...
    for (vector<int64_t>::iterator it = vecTemp.begin(); it != vecTemp.end(); it++)
    {

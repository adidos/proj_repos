...
int GameRoomConfig::loadGameConfig(const string& file_name)
{
	...
	if(0 != rst)
	{
		LOG4CPLUS_ERROR(ALogger, "load config file " << file_name << " failed...");
		return -1;
	}
...
int GameRoomConfig::room_list(vector<RoomIdent>& room_array)
{
	...
	if(strGames.empty())
	{
		LOG4CPLUS_ERROR(ALogger, "game list is not config.");
		return -1;
	}
	...
	for(; game_iter != game_id_array.end(); ++game_iter)
	{
		...
		if(strRooms.empty())
		{
			LOG4CPLUS_ERROR(ALogger, "game "<< *game_iter<< " rooms list is null");
			continue;
		}
		...
		vector<GameRoomInfo>& room_info_array)
{
	...
	{	
		...
		LOG4CPLUS_INFO(ALogger, "RoomInfo: game_id = " << room_info.game_id

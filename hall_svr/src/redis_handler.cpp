...
IMPL_LOGGER(RedisHandler, logger);
...
int RedisHandler::push_queue(const string& queue, const string& value) {
	if(queue.empty() || value.empty()){
		LOG4CPLUS_ERROR(logger, "PUSH QUEUE INVALID PARAM");
		return -1;
	}
	...
	if(REDIS_ERR_IO == context_->err){
		...
		//如果是网络io错误，进行重新连接
		LOG4CPLUS_ERROR(logger, "net error occur, err: " << context_->errstr << ", reconnect...");
		int rst = conn2Redis();
		if(0 != rst) {
			LOG4CPLUS_ERROR(logger, "Reconnect server failed, err: " << context_->errstr);
			return -1;	
		}
		...
		freeReplyObject(replay);
		LOG4CPLUS_ERROR(logger, "RPUSH: failed, err: " << context_->errstr);
		return context_->err;
	}

	LOG4CPLUS_DEBUG(logger, "RPUSH: push " << value << " into queue[" << queue 
...
int RedisHandler::pop_queue(const string& queue, string& value) {
	if(queue.empty()) {
		LOG4CPLUS_ERROR(logger, "POP QUEUE INVALID PARAM");
		return -1;
	}
	...
	if(REDIS_ERR_IO == context_->err){
		...
		LOG4CPLUS_ERROR(logger, "POP : failed, err: " << context_->errstr);
		freeReplyObject(replay);
		return context_->err;
	}
	...
	LOG4CPLUS_DEBUG(logger, "POP: pop " << value << " from queue[" << queue << "]");
...
{
	if(key.empty()) {
		LOG4CPLUS_ERROR(logger, "GET VALUE INVALID PARAM");
		return -1;
	}
	...
	if(REDIS_ERR_IO == context_->err){
		...
		LOG4CPLUS_ERROR(logger, "GET : failed, err: " << context_->errstr);
		freeReplyObject(replay);
		return context_->err;
	}
	...
	LOG4CPLUS_DEBUG(logger, "Get: get " << key << ", value " << value << "]");
...
int RedisHandler::set_value(const string& key, const string& value){
	...
	if(REDIS_ERR_IO == context_->err){
		...
		LOG4CPLUS_ERROR(logger, "SET: failed, err: " << context_->errstr);
		freeReplyObject(replay);
		return context_->err;
	}
		
	LOG4CPLUS_DEBUG(logger, "SET: set " << key << " \t value " << value );
...
int RedisHandler::conn2Redis() {
	...
	if(NULL == context_ || context_->err)
	{
		if(NULL != context_)
		{
			LOG4CPLUS_ERROR(logger, "connect to redis server " << host_ << ":"
...
{	
	...
	if(REDIS_ERR_IO == context_->err){
		...
		LOG4CPLUS_ERROR(logger, "subscribe : failed, err: " << context_->errstr);
		freeReplyObject(replay);
		return context_->err;
	}				
...
int RedisHandler::unsubscribe_channel( const string & channel_name )
{
	if(channel_name.empty()){
		LOG4CPLUS_ERROR(logger, "unsubscribe_channel INVALID PARAM");
		return -1;
	}

...
bool GetNoticesHandler::handle(CmdTask& task)
{
	...
	if(0 != rst)
	{
		LOG4CPLUS_ERROR(FLogger, "ckeck command failed. ident = "
	...
	if(!bSuccess)
	{
		LOG4CPLUS_ERROR(FLogger, "decode param from idatax failed.");
		return false;
	}

	LOG4CPLUS_DEBUG(FLogger, "Get Notices: game_id :" << game_id << ", channal: " << channal << ", version: " << version);

	vector<Notice> notice_array;
	NoticeConfig* ptr = NoticeConfig::getInstance();
	ptr->getNotices(channal, version, notice_array);
	LOG4CPLUS_DEBUG(FLogger, "get notice cnt =" << notice_array.size());
	...
	for(; iter != notice_array.end(); ++iter)
	{
		...
		++idx;
		LOG4CPLUS_INFO(FLogger, "Notice info: { id :" << iter->id << ", type: " 

...
bool CheckinHandle::handle(CmdTask& task)
{
	...
	if(0 == ret && 0 == checkin_ret.result)
	{
		...
		else
		{
			...
			LOG4CPLUS_ERROR(CLogger::logger, "Uesr " << uid << " checkin success, but"
	...
	DataXCmd* pResp = new DataXCmd("CheckInResp", pCmd->get_cipher_flag());
...
int CheckinHandle::doCheckin(int game_id ,int64_t uid, CheckinResult& ret)
{
	...
	LOG4CPLUS_ERROR(CLogger::logger, "user checkin call result is  " << result 
		<< ", return " << ret.dump());
...
int CheckinHandle::handleCheckin(CheckinResult result, UpdateGameInfoResp& resp)
{
	...
	LOG4CPLUS_DEBUG(CLogger::logger, result.dump() << " do check in request"
...
int CheckinHandle::initRewards()
{
	...
	LOG4CPLUS_DEBUG(CLogger::logger, "check reward configue: ");
	for(size_t i = 0; i < days.size(); ++i)
	{
		...
		LOG4CPLUS_DEBUG(CLogger::logger, "{" << value << " : " << reward << "}");
		...
		LOG4CPLUS_DEBUG(CLogger::logger, item.dump());	
